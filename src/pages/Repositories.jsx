...
export default function Repositories() {
  ...
  const filtered = useMemo(() => {
    return sources.filter((s) => {
      ...
      const matchesTheme = !selectedTheme || s.theme === selectedTheme;
      const matchesDataType = !selectedDataType || s.data_types?.includes(selectedDataType);

      return matchesSearch && matchesTheme && matchesDataType;
    });
    ...
    <div className="pt-14">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          ...
          {showFilters && (
            ...
            >
              ...
              {activeFilters > 0 && (
                <button
                  onClick={() => { setSelectedTheme(""); setSelectedDataType(""); }}
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <p className="font-heading text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-8">
          {loading ? "Carregando..." : `${filtered.length} fonte${filtered.length !== 1 ? "s" : ""} encontrada${filtered.length !== 1 ? "s" : ""}`}
        </p>
